...
public class MinumanAdapter extends RecyclerView.Adapter<MinumanAdapter.Holder> {
    ...
    private ArrayList<MenuModel> minumanSelect;
    private MinumanSelect listener;

    public MinumanAdapter(ArrayList<MenuModel> modelList, MinumanSelect listener) {
        this.modelList = modelList;
        this.listener = listener;
        minumanSelect = new ArrayList<>();
    }
    ...
    private void updatePesan(int id, int jum, int pos){
        if (jum!=0){
            ...
            minumanSelect.set(pos, modelPesan);
        }
        listener.resultMinuman(minumanSelect);
    }

    public interface MinumanSelect {
        void resultMinuman(ArrayList<MenuModel> listPesan);
    }
}
